...
import { Avatar, Button, FormControl, Grid, IconButton, InputAdornment, InputLabel, OutlinedInput, TextField, Typography } from "@mui/material"

import useStyles from "../../../../../../styles/AdvancedMapPage/GoogleMaps/EditProperties"
import toolsUtils from "../../../../../../utils/ToolsUtils"
import theme from "../../../../../../styles/Utils/Theme"


const EditProperties = ({ handleUser, singleUser }) => {
  const { classes } = useStyles()

  const [value, setValue] = useState<string>("Melhores dias para ligações: segundas e terça-feiras. Gosta de vinho tinto seco Cabernet Sauvignon. ")
  const [copiedPhoneNumber, setCopiedPhoneNumber] = useState<boolean>(false)
    ...
    <Grid>
      <Grid item xs={2} style={{ textAlign: "right", width: "10%", marginRight: "8px" }}>
        <IconButton size="small" onClick={() => handleUser("person")}>
          <ArrowBackIosIcon style={{ color: theme.colors.onPrimaryContainer }} fontSize="small" />
        </IconButton>
      </Grid>
      <Grid>
        <Grid className={classes.containerUser}>
          ...
          <Grid>
            <Typography className={classes.userName}>{singleUser.name} {singleUser.surname}</Typography>
            <Typography className={classes.userRole}>{singleUser.isowner ? "Proprietário" : "Representante Comercial"}</Typography>
          </Grid>
        </Grid>
        <Grid className={classes.containerBoxUser}>
          <Typography className={classes.propertyDescriptionEdit}>
          ...
          <Grid className={classes.propertieDetail}>
            <FormControl className={classes.textField} variant="outlined">
              <InputLabel htmlFor="outlined-adornment-password">Telefone</InputLabel>
              <OutlinedInput
          ...
          <Grid className={classes.boxDetailButton}>
            ...
            <Button onClick={() => handleUser("activateEditPerson")}>Editar usuário</Button>
          </Grid>
        </Grid>
      </Grid>
    </Grid>
